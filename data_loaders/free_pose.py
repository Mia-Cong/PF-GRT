...
from .base_utils import pose_inverse, transform_points_Rt
import numpy as np

def gen_relative_pose(canonical_pose , abs_pose):
    pose_inv = inv(canonical_pose)
    return np.matmul(abs_pose, pose_inv )  # (4, 4)

def interpolate_render_poses(database, view_num, loop=True):
    poses = database.render_poses
    poses = [pose[:3,:] for pose in poses]
...
def forward_circle_poses(database):
    poses = database.render_poses
    poses = [pose[:3,:] for pose in poses]
    poses_inv = [pose_inverse(pose) for pose in poses]
    cam_pts = np.asarray(poses_inv)[:, :, 3]
    ...
    depth_ranges = database.render_depth_range
    ...
    render_poses = render_path_spiral(avg_pose_inv,avg_down,range_in_avg_pose, mean_dz, 0., 1, 66)
    render_poses = [pose_inverse(pose) for pose in render_poses]
    render_poses = np.asarray(render_poses)
    return render_poses

def interpolate_poses(database):
...
def get_render_poses(database, pose_type='circle', pose_fn=None):
